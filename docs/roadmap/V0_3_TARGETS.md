...
| Cline       | Exact `--model` value | Literal argument or deterministic argument template | Required visible template binding  | Visible Cline `--plan` behavior | Direct one-shot execution without generated bypass flags | Not normalized    | Not exposed |
...
Cline 3.0.39 consumes piped stdin only when `--json` event output is enabled.
Because v0.3 preserves Cline's ordinary text response artifact, connected text
is composed into the visible positional prompt through a deterministic template
instead. Legacy v0.3 Cline blocks with a stdin context binding are migrated to
that template before preflight and execution.
...
- Pass the final resolved current-directory or run-worktree path through
  `--add-dir`. Headless `agy --print` otherwise selects its global scratch
  workspace even when the child process cwd is correct; Vorchestra must not
  create persistent Antigravity projects to correct that behavior.
...
- Allow a runtime that declares model-selection capability to expose an optional
  per-block model override.
- Load per-runtime model choices and optional defaults only from the user-owned
  `~/.vorchestra/models.json`; do not compile provider model identifiers into
  the application.
- Apply a machine-local configured default at preflight and execution only when
  a block has no explicit model. Keep Custom available for exact identifiers.
- Store an explicit override in the selected block's portable invocation.
...
- Vorchestra does not generate autonomous approval, sandbox, or permission
  bypass flags for Codex, Cline, or Antigravity. Codex uses
  `--skip-git-repo-check` to support explicitly reviewed workflows in ordinary
  directories without changing the selected sandbox.
...
- automatic approval, sandbox, or permission bypasses;
